...
echo
ls -1 ../ct/src/resources/server-provision*.json
ls -1 server-provision*.json
echo
...
set -x
curl -v -XPOST --http2-prior-knowledge -d @${file} -H "Content-Type: application/json" http://localhost:8074/provision/v1/server-provision
