...
@pytest.mark.admin
def test_001_i_want_to_identify_wrong_schema_for_server_provision_operation_on_admin_interface(admin_server_provision):

  admin_server_provision({ "foo": "bar" }, responseBodyRef = INVALID_SERVER_PROVISION_SCHEMA__RESPONSE_BODY, responseStatusRef = 400)
...
@pytest.mark.server
def test_008_i_want_to_check_fullmatchingregexreplace_on_traffic_interface(admin_server_matching, admin_server_provision, h2ac_traffic):
  ...
  admin_server_matching(algorithm)
...
def test_009_i_want_to_check_prioritymatchingregex_on_traffic_interface(admin_cleanup, admin_server_provision, h2ac_traffic):
  ...
  # Provisions
  admin_server_provision(string2dict(REGEX_FOO_BAR_SERVER_PROVISION_TEMPLATE, id=55500, trailing=4))
  admin_server_provision(string2dict(REGEX_FOO_BAR_SERVER_PROVISION_TEMPLATE, id=5551122, trailing=2))
  admin_server_provision(string2dict(REGEX_FOO_BAR_SERVER_PROVISION_TEMPLATE, id=555112244, trailing=0))
...
@pytest.mark.server
def test_010_i_want_to_get_answer_for_default_provision_on_traffic_interface(admin_server_provision, h2ac_traffic):

  # Provision
  admin_server_provision(string2dict(FALLBACK_DEFAULT_SERVER_PROVISION))
...
def test_011_multiple_provisions_operation(admin_cleanup, admin_server_provision, h2ac_admin):
  ...
  admin_server_provision(provisions, responseBodyRef=VALID_SERVER_PROVISIONS__RESPONSE_BODY)
