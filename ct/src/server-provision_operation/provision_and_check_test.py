import pytest
import json
from conftest import ADMIN_SERVER_PROVISION_URI, INVALID_PROVISION_SCHEMA__RESPONSE_BODY, VALID_PROVISIONS__RESPONSE_BODY
...
def test_001_i_want_to_identify_wrong_schema_for_server_provision_operation_on_admin_interface(admin_server_provision):
  ...
  admin_server_provision({ "foo": "bar" }, responseBodyRef = INVALID_PROVISION_SCHEMA__RESPONSE_BODY, responseStatusRef = 400)
...
def test_009_i_want_to_check_prioritymatchingregex_on_traffic_interface(admin_cleanup, admin_server_provision, h2ac_traffic):
  ...
  admin_server_provision(string2dict(REGEX_FOO_BAR_PROVISION_TEMPLATE, id=5551122, trailing=2))
  admin_server_provision(string2dict(REGEX_FOO_BAR_PROVISION_TEMPLATE, id=555112244, trailing=0))
