...
def test_001_i_want_to_get_internal_data_on_admin_interface(resources, h2ac_admin, h2ac_traffic):
  ...
  # Send GET
  response = h2ac_admin.get("/provision/v1/server-data")
  responseBodyRef = [{'method': 'GET', 'requests': [{'body': {'foo': 'bar-1'}, 'receptionTimestampMs': 1623420758129, 'state': 'initial'}], 'uri': '/app/v1/foo/bar/1'}]
...
def test_002_i_want_to_get_speficic_internal_data_on_admin_interface(resources, h2ac_admin, h2ac_traffic):
  ...
  responseBodyRef = {'method': 'GET', 'requests': [{'body': {'foo': 'bar-5'}, 'receptionTimestampMs': 1623420758129, 'state': 'initial'}], 'uri': '/app/v1/foo/bar/5'}
