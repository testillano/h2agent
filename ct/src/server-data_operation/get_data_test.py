...
def test_002_i_want_to_get_speficic_internal_data_on_admin_interface(h2ac_admin, h2ac_traffic, admin_provision):
  ...
  responseBodyRef = [{'previousState': 'initial', 'receptionTimestampMs': 1623552779114, 'responseBody': {'foo': 'bar-2'}, 'responseDelayMs': 0, 'responseHeaders': {'content-type': 'text/html', 'x-version': '1.0.0'}, 'responseStatusCode': 200, 'serverSequence': 35, 'state': 'initial'}]
  # Response will be something like: {"method":"GET","requests":[{"body":{"foo":"bar-2"},"receptionTimestampMs":1623439631262,"state":"initial"}
  # We have a variable field 'receptionTimestampMs' and we don't know its real value.
  # So, we will remove it from both reference and response python dictionaries:
  # In this test we request specific server-data, so we don't have array:
  del response["body"][0]["serverSequence"]
  del response["body"][0]["receptionTimestampMs"]
  del responseBodyRef[0]["serverSequence"]
  del responseBodyRef[0]["receptionTimestampMs"]
  h2ac_admin.assert_response__status_body_headers(response, 200, responseBodyRef)
